...
Algorithmically, inspiration was taken from minisat and glucose projects. The solver implements the following concepts:
...
IPASIR header in order to define compatible interface, however the compatibility level support can be dropped at compile time.
...
and depend on a number of libraries available in PyPI. Testing scripts run solver with formulas in DIMACS format 
and then validate solver output using CaDiCaL SAT solver.
...
Some of CNF files in `tests/samples` directory are generated using cnfgen utility and contain
